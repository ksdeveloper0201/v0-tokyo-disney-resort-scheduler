...
const typeColors: Record<ItemType, { bg: string; border: string; text: string; badge: string; solid: string }> = {
  attraction: { bg: 'bg-primary/15', border: 'border-primary/40', text: 'text-primary', badge: 'bg-primary text-primary-foreground', solid: 'bg-primary' },
  show: { bg: 'bg-chart-2/15', border: 'border-chart-2/40', text: 'text-chart-2', badge: 'bg-chart-2 text-white', solid: 'bg-chart-2' },
  parade: { bg: 'bg-chart-3/15', border: 'border-chart-3/40', text: 'text-chart-3', badge: 'bg-chart-3 text-white', solid: 'bg-chart-3' },
  event: { bg: 'bg-chart-4/15', border: 'border-chart-4/40', text: 'text-chart-4', badge: 'bg-chart-4 text-white', solid: 'bg-chart-4' },
  restaurant: { bg: 'bg-accent/15', border: 'border-accent/40', text: 'text-accent', badge: 'bg-accent text-white', solid: 'bg-accent' },
}
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`font-medium text-xs truncate ${isExpanded ? '' : 'max-w-[80px]'}`}>
                      {slot.item.name}
                    </span>
                    {slot.isLocked && <Lock className="w-3 h-3 text-muted-foreground flex-shrink-0" />}
                  </div>
  ...
  return (
    ...
    >
      ...
      <div className="p-2 border-b border-border bg-muted/50 flex items-center justify-between">
        <span className="text-xs font-medium text-muted-foreground">タイムライン</span>
        <button
          onClick={onExpand}
    ...
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur border-b border-border p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold">1日のタイムスケジュール</h2>
          <button
            onClick={onClose}
...
function DragOverlayContent({ item }: { item: ScheduleItem }) {
  const colors = typeColors[item.type]
  return (
    <div className={`p-3 rounded-lg ${colors.bg} ${colors.border} border-2 shadow-xl min-w-[200px]`}>
      <div className="flex items-center gap-2">
...
export function ScheduleBuilder({ onBack, onSave }: ScheduleBuilderProps) {
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    // Category reordering
    if (categoryOrder.includes(active.id as ItemType) && categoryOrder.includes(over.id as ItemType)) {
    ...
    // Item dropped on timeline
    if (active.data.current?.type === 'new-item' && over.data.current?.type === 'timeline-slot') {
    ...
    // Item dropped on mini timeline
    if (active.data.current?.type === 'new-item' && over.id === 'mini-timeline') {
      const item = active.data.current.item as ScheduleItem
      
      if (!isFixedTimeItem(item)) {
        // Show time picker for free time items dropped on mini timeline
        setTimeModalItem(item)
        setTimeModalHour(null)
        setShowTimeModal(true)
      }
    }
  }, [categoryOrder, setCategoryOrder, selectedItems, toggleSelectedItem, addScheduleSlot])
